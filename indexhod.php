<?php include('connecthod.php');
	...
	if($epr=='save') {
		...
		$a_sql=mysql_query("INSERT INTO app_formhod (name,dept,designation,type,days,date,adjust) VALUES('$name', '$dept', '$designation', '$type', '$days', '$date', '$adjust')");
		
		if($a_sql)
			header("location:indexhod.php");
		else
			$msg='Error: '.mysql_error();
		$b_sql=mysql_query("INSERT INTO adjusthod (department,year,sem,section,session,period,faculty) VALUES('$deprtment', '$year', '$sem', '$section', '$session', '$period', '$faculty')");
		if($b_sql)
			header("location:indexhod.php");
		else
			$msg='Error: '.mysql_error();
	...
	if($epr=='delete') {
		$id=$_GET['id'];
		$delete=mysql_query("DELETE FROM app_formhod WHERE id=$id");
		if($delete)
			header("location:indexhod.php");
		else 
			$msg='Error: '.mysql_error();
	...
	if($epr=='saveup') {
		...
		$adjust=$_POST['adjust'];
		$id=$_POST['id'];
		$a_sql=mysql_query("UPDATE app_formhod SET name='$name', dept='$dept', designation='$designation', type='$type', days='$days', date='$date', adjust='$adjust' WHERE id='$id'");
		if($a_sql)
...
<html>
	...
	<ul align=left;>
		<li><a href="profilehod.php">Profile</a></li>
		<li><a href="indexhod.php">Apply Leave</a></li>
		<li><a href="statushod.php">Status</a></li>
	 ...
	 <!-- +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ -->	
		...
		if($epr=='update'){
			$id=$_GET['id'];
			$row=mysql_query("SELECT * FROM app_formhod WHERE id='$id'");
...
<!-- +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++= -->
		<h2 align="center">Apply Leave</h2>
			...
			<form method="POST" action='indexhod.php?expr=save'>
				<table align="center">
					<tr>
						<td>Name:</td>
		...
		<table align="center" border="1" cellspacing="0" cellpadding="0" width="1000">
				...
				<?php
				$sql=mysql_query("SELECT * FROM adjusthod where 1=1 order by id desc limit 1");
				$i=1;
		...
		<!--<h2 align="center">form</h2>-->
			<table align="center" border="1" cellspacing="0" cellpadding="0" width="1000">
				...
				<?php
				$sql=mysql_query("SELECT * FROM app_formhod where 1=1 order by id desc limit 1");
				$i=1;
					while ($row=mysql_fetch_array($sql)) {
						echo "<tr>
							<td>".$i."</td>
